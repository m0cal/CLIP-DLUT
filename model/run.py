...
from modules.residual_lut import ResidualLUT
...
def monotonicity_loss(lut):
    ...
    loss_smooth = (ddx.square().mean() + ddy.square().mean() + ddz.square().mean()) * 10.0
...
def run(image: Tensor, 
    ...
    # 替换 MLP 为直接优化的 Residual LUT
    lut_model = ResidualLUT(dim=33).to(device)
    ...
    # 直接优化 lut_model 的参数 (即 residual lut tensor)
    optimizer = optim.AdamW(lut_model.parameters(), lr)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=iteration, eta_min=1e-5)
    ...
    for step in range(iteration):
        # 这里的 forward 很简单：Identity + 学习到的 Residual
        updated_lut = lut_model(identity_lut)
        ...
        target_vol = original_vol * 0.85
        loss_volume = torch.relu(target_vol - current_vol).mean() * 100.0

        # Residual TV Loss: 强制 Residual 局部平滑，作为"胶水"粘合被 sparse gradient 更新的顶点
        res = lut_model.lut_residual
        tv_dim1 = torch.mean((res[:, :, 1:, :, :] - res[:, :, :-1, :, :]) ** 2)
        tv_dim2 = torch.mean((res[:, :, :, 1:, :] - res[:, :, :, :-1, :]) ** 2)
        tv_dim3 = torch.mean((res[:, :, :, :, 1:] - res[:, :, :, :, :-1]) ** 2)
        loss_tv = (tv_dim1 + tv_dim2 + tv_dim3) * 1000.0 

        loss = total_clip_loss + mono_loss + loss_volume + loss_tv
        ...
        # 打印 Residual 的梯度统计
        if lut_model.lut_residual.grad is not None:
             grad_mean = lut_model.lut_residual.grad.abs().mean().item()
             grad_max = lut_model.lut_residual.grad.abs().max().item()
             print(f"Residual LUT | Grad Mean: {grad_mean:.8f} | Max: {grad_max:.8f}")
        else:
             print("Residual LUT | No Grad")

        print("-" * 30)
        optimizer.step()
        scheduler.step()

        print(f"Step {step}, Loss: {loss.item():.6f} (CLIP: {total_clip_loss.item():.6f}, Mono: {mono_loss.item():.6f}, Vol: {loss_volume.item():.6f}, TV: {loss_tv.item():.6f})")
...
if __name__ == "__main__":
    ...
    # 使用较大的学习率 (1e-2), 因为我们在直接优化 LUT 值而非 MLP 权重
    run(img, "赛博朋克风格的巷子", original_text="巷子", lr=1e-2, iteration=1500)
